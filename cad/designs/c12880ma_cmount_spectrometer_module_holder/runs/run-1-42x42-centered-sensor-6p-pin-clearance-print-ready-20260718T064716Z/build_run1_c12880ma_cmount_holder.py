#!/usr/bin/env python3
"""Build the first print-ready C12880MA module C-mount holder."""
...
STEM = "c12880ma_cmount_holder_42x42_run1"
...
PARAMS = {
    ...
    "six_pin_row_position_note": (
        "Photo-derived centerline about 3.2 mm above the measured PCB bottom edge. "
        "The cut is intentionally generous and connected; verify this one Z offset "
...
def build_assembly() -> cq.Assembly:
    ...
    assembly.add(
        build_holder_plate(),
        name="independent_42x42_sensor_holder_plate",
        color=cq.Color(0.33, 0.35, 0.38, 1.0),
    )
    assembly.add(
        build_board_proxy(),
        name="c12880_module_pcb_proxy_38p3x22p8_offset_from_sensor_axis",
...
def draw_rear_alignment(path_base: Path) -> None:
    geometry = board_geometry()
    bounds = geometry["pcb_bounds_relative_to_sensor_axis_mm"]
    ...
    ax.set_title("C12880MA module holder: rear alignment (looking toward C-mount)")
    ...
    for suffix in (".png", ".pdf", ".svg"):
        fig.savefig(path_base.with_suffix(suffix), bbox_inches="tight")
    plt.close(fig)
...
def draw_side_section(path_base: Path) -> None:
    ...
    ax.set_xlim(-1.0, rear + PARAMS["pcb_thickness_x_mm"] + 2.0)
    ax.set_ylim(-23.0, 23.0)
    ax.set_aspect("equal", adjustable="box")
    ax.set_xlabel("Optical axis X (mm)")
    ax.set_ylabel("Cross-section span (mm)")
    ax.set_title("C12880MA holder side section and thread/package separation")
...
def write_readme(
    ...
    path.write_text(
        f"""# C12880MA 42 x 42 C-Mount Module Holder
...
The row center is photo-derived at Z=`{PARAMS['six_pin_row_center_z_mm']} mm`,
...
blender --background --python {repo_path(path.parent / 'render_run1_c12880ma_cmount_holder.py')}
```
...
def main() -> None:
    ...
    alignment_base = ARTIFACT_DIR / f"{STEM}_rear_alignment"
    ...
    draw_rear_alignment(alignment_base)
    ...
    outputs = {
        ...
        "rear_alignment_png": repo_path(alignment_base.with_suffix(".png")),
        "rear_alignment_pdf": repo_path(alignment_base.with_suffix(".pdf")),
        "rear_alignment_svg": repo_path(alignment_base.with_suffix(".svg")),
