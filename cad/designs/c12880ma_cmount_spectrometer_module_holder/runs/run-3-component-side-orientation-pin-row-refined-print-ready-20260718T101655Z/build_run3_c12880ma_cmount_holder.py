#!/usr/bin/env python3
"""Build run 3 with the verified component-side module orientation."""
...
PARAMS = {
    ...
    "orientation_correction": (
        "The component-side view is authoritative: sensor package up, 6P socket "
        "down, sensor closer to the left PCB edge, and more board margin on the "
        "right. Front/rear viewing changes must not be baked into the physical Y/Z "
        "feature coordinates. The PCB mounts behind the plate while the sensor "
        "package projects toward the C-mount."
    ),
    ...
    "six_pin_row_position_note": (
        "The socket stays below the sensor at Z=-9.3 mm. The measured end margins "
        "are about 9 mm on the sensor-near/left PCB side and 17 mm on the right. "
        "With 2.54 mm pitch, a Y=-0.4 mm row center gives 8.8 and 16.8 mm. The "
        "3.0 mm relief around a roughly 1.0 mm pin provides 1.0 mm radial clearance."
    ),
    "print_orientation": (
...
def geometry_checks() -> dict[str, object]:
    ...
    checks = {
        ...
        "component_side_orientation_is_sensor_up_socket_down": (
            math.isclose(PARAMS["pcb_center_z_from_sensor_axis_mm"], -1.1)
            and math.isclose(PARAMS["six_pin_row_center_z_mm"], -9.3)
            and math.isclose(bounds["z_min"], -12.5)
            and math.isclose(bounds["z_max"], 10.3)
        ),
        "component_side_pcb_center_z_mm": PARAMS["pcb_center_z_from_sensor_axis_mm"],
        "component_side_six_pin_row_z_mm": PARAMS["six_pin_row_center_z_mm"],
        "component_side_sensor_is_closer_to_left_edge": (
...
def draw_component_side_alignment(path_base: Path) -> None:
    ...
    ax.set_title("C12880MA holder: component-side alignment (sensor up, socket down)")
...
def write_readme(
    ...
    path.write_text(
        f"""# C12880MA 42 x 42 C-Mount Module Holder - Verified Orientation
...
The socket row remains below the sensor at Z=`{PARAMS['six_pin_row_center_z_mm']} mm`.
...
blender --background --python {repo_path(path.parent / 'render_run3_c12880ma_cmount_holder.py')}
```
...
def main() -> None:
    ...
    required_checks = [
        ...
        geometry_validation["component_side_orientation_is_sensor_up_socket_down"],
        geometry_validation["component_side_sensor_is_closer_to_left_edge"],
