...
STEM = "as7343_cmount_spectral_module_holder_direct_socket_pcb_pin_holes"
...
def build_sensor_plate_body() -> cq.Workplane:
    ...
    holder = holder.cut(board_pocket_cutter()).cut(pin_header_hole_clearance_cutter())
    ref = board_reference_geometry()
...
def write_readme(path: Path, outputs: dict[str, str]) -> None:
    ...
    path.write_text(
        f"""# AS7343 C-Mount Spectral Module Holder Direct Socket PCB Pin Holes
