#!/usr/bin/env python3
"""Render the Lumileds PCB-aligned cage holder with rear PCB sink."""
...
def setup_scene() -> None:
    ...
    materials = {
        "holder": make_material("matte graphite printed holder", (0.34, 0.34, 0.31, 1.0), roughness=0.66),
        "pcb": make_material("green PCB seated in 1.6 mm sink", (0.02, 0.42, 0.12, 0.72), roughness=0.48),
        "sink": make_material("cyan sink outline proxy", (0.10, 0.70, 0.82, 0.24), roughness=0.42),
    ...
    holder.name = "single_piece_holder_with_1p6mm_rear_pcb_sink"
    holder.data.materials.clear()
    holder.data.materials.append(materials["holder"])

    rear_face_z = -P["body_thickness"] / 2.0
    pcb_center_z = rear_face_z + P["pcb_thickness"] / 2.0 + 0.02
    add_cylinder_z("24 mm PCB proxy seated flush in rear sink", P["pcb_diameter"] / 2.0, P["pcb_thickness"], (0, 0, pcb_center_z), materials["pcb"])
    add_cylinder_z("24.4 mm sink opening reference", P["pcb_sink_diameter"] / 2.0, 0.05, (0, 0, rear_face_z + 0.03), materials["sink"])
    add_cylinder_z("LED emitter at PCB center", 2.35, 0.8, (0, 0, rear_face_z - 0.42), materials["led"])
    ...
    for index, (x, y) in enumerate(P["m2_points"], start=1):
        add_cylinder_z(f"M2 PCB alignment hole {index}", 1.05, P["body_thickness"] + P["pcb_thickness"] + 2.0, (x, y, -0.8), materials["pin"], vertices=48)
    ...
    add_box("right-angle 2P header body proxy", (5.2, 6.4, 2.5), (12.2, -0.27, rear_face_z - 1.25), materials["header"])

    bpy.ops.object.light_add(type="AREA", location=(0, -35, -45))
    light = bpy.context.object
    light.name = "rear softbox"
    light.data.energy = 1550
    light.data.size = 6.0
    bpy.ops.object.light_add(type="AREA", location=(-32, 26, 34))
