...
All three arms use the same `JH036 cemented achromatic doublet` lens. The beam-splitter center is the fixed datum `(255, 210, 600) mm`. The CAD places the A, B, and C holder contact planes one catalog EFL from that datum: `45.99900 mm`. Therefore A-B and A-C nominal mechanical-seat separations are `2f = 91.99800 mm` under the source OpenHI thin-lens convention. The catalog does not provide BFL/principal-plane locations.

Here "nominal lens plane" means the inward annular mechanical seat used by the original OpenHI assembly. The inward optical-axis vertex is `45.81373 mm` from the beam splitter in this modeled geometry. Depending on the inward surface, that vertex and the annular seat may differ. Neither datum should be called a certified thick-lens principal plane without the complete vendor prescription.
