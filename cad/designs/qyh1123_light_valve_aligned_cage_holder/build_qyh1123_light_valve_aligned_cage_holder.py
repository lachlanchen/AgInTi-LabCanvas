#!/usr/bin/env python3
"""Build a clean 30 mm cage holder for the QYH1123 LCD light valve.

The optical origin is the 15 x 15 mm visible aperture, not the physical glass
center. The QYH1123 drawing shows an 18 x 20 x 2 mm body and a 15 x 15 mm V.A.
window. The left active-area margin is dimensioned as 0.60 mm, so the active
window center is 0.90 mm left of the glass body center.
"""
...
STEM = "qyh1123_light_valve_aligned_cage_holder"
...
def build_holder() -> cq.Workplane:
    ...
    holder = holder.cut(z_box(relief, (pocket_center_x, relief_center_y, front_z - relief[2] / 2.0 + 0.04)))
...
def write_alignment_svg(path: Path) -> None:
    ...
    legend = [
        ...
        "Pocket/sink: +0.4 mm XY clearance, 2.2 mm deep",
        f"Terrace lip: {p['terrace_lip_per_side_mm']} mm per side inside the V.A.",
        "Drawing offset: active center is 0.9 mm left of body center",
...
def write_readme(path: Path, outputs: dict[str, str]) -> None:
    ...
    light_w, light_h = light_window_xy()
    path.write_text(
        f"""# QYH1123 Light-Valve Aligned Cage Holder
...
- LCD sink: `{PARAMS['valve_outer_width_mm'] + PARAMS['valve_pocket_clearance_mm']} x {PARAMS['valve_outer_height_mm'] + PARAMS['valve_pocket_clearance_mm']} x {PARAMS['valve_pocket_depth_mm']} mm`.
