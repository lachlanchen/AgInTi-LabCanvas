#!/usr/bin/env python3
"""Build run 4: M6-fit cage rod dock with full ears and 10 mm side contact."""
...
def full_corner_ear(sx: int, sy: int) -> cq.Workplane:
    """Filled ear with real 10 mm side-edge contact plus a diagonal pull."""
    p = PARAMS
    ...
    # overlap into the dock. Build the ear from simple filled pieces instead of
    # one deeply concave polygon, because the concave version can confuse the
    # CadQuery boolean and drop the 30 mm dock body.
    ...
    ear = z_poly(local_points(diagonal_corner_pull), thickness, 0.0)
    ear = ear.union(z_poly(local_points(side_contact_a), thickness, 0.0))
    ear = ear.union(z_poly(local_points(side_contact_b), thickness, 0.0))
    return ear
...
def write_top_svg(path: Path) -> None:
    ...
    for sx_sign in (-1, 1):
        for sy_sign in (-1, 1):
            ...
            def local_points(local: list[tuple[float, float]]) -> list[tuple[float, float]]:
                return [(sx_sign * (half_w + u), sy_sign * (half_h + v)) for u, v in local]
            ...
            lines.append(poly(local_points(side_contact_a), "#fff7ed", "#dd6b20"))
            lines.append(poly(local_points(side_contact_b), "#fff7ed", "#dd6b20"))
            lines.append(poly(local_points(diagonal_corner_pull), "#fef5e7", "#dd6b20"))
    lines.append(
    ...
    legend = [
        ...
        f"Ears: {p['anti_warp_ear_thickness_mm']:.1f} mm thick, filled polygon",
        f"Side contact: {p['anti_warp_ear_side_contact_length_mm']:.0f} mm along each adjacent edge",
...
def write_readme(path: Path, outputs: dict[str, str], checks: dict[str, object]) -> None:
    ...
    path.write_text(
        f"""# Run 4: M6 Dock With 10 mm Side-Contact Ears
...
Each ear overlaps the two side edges by `{PARAMS['anti_warp_ear_side_contact_length_mm']} mm`.
...
- Side contact: `{PARAMS['anti_warp_ear_side_contact_length_mm']} mm` along each adjacent side.
...
def render_with_blender() -> None:
    ...
    if not blender:
        ...
        return
    subprocess.run(
        [blender, "--background", "--python", str(DESIGN_DIR / "render_run4_m6_strong_ears_10mm_side_contact.py")],
        check=True,
    )
