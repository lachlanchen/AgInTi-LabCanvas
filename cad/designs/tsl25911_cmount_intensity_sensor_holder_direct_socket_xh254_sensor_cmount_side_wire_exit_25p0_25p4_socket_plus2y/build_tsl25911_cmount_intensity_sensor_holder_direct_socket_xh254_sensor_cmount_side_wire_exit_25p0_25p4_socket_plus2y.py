#!/usr/bin/env python3
"""Build a direct-socket nominal 25.4 mm C-mount holder for a TSL25911 module."""
...
STEM = "tsl25911_cmount_intensity_sensor_holder_direct_socket_xh254_sensor_cmount_side_wire_exit_25p0_25p4_socket_plus2y"
...
def board_reference_geometry() -> dict[str, object]:
    ...
    socket_y_min = (
        ...
        - socket_depth
        - socket_clearance
        - PARAMS["xh254_socket_relief_expand_toward_sensor_side_y_mm"]
    )
    socket_y_max = bounds["y_max"] + PARAMS["xh254_socket_relief_extra_y_mm"] + socket_clearance
    nominal_socket_y_min = bounds["y_max"] - socket_depth
    nominal_socket_y_max = bounds["y_max"]
    socket = {
        "name": "XH2.54_5P_socket_relief",
        "assumed_side": PARAMS["connector_side"],
        "y_min": round(socket_y_min, 4),
        "y_max": round(socket_y_max, 4),
        ...
        "extra_clearance_toward_sensor_side_y_mm": PARAMS["xh254_socket_relief_expand_toward_sensor_side_y_mm"],
        "nominal_socket_body_y_min": round(nominal_socket_y_min, 4),
        "nominal_socket_body_y_max": round(nominal_socket_y_max, 4),
        "nominal_body_mm": {
...
def xh254_socket_proxy() -> cq.Workplane:
    ...
    center_y = (socket["nominal_socket_body_y_min"] + socket["nominal_socket_body_y_max"]) / 2.0
...
def write_alignment_svg(path: Path) -> None:
    ...
    legend = [
        ...
        f"Socket/wire relief starts {PARAMS['xh254_socket_relief_expand_toward_sensor_side_y_mm']} mm farther toward the sensor-side edge for deeper PCB seating",
...
def write_readme(path: Path, outputs: dict[str, str]) -> None:
    ...
    path.write_text(
        f"""# TSL25911 C-Mount Intensity Sensor Holder 25.0/25.4
...
- Expand the socket/wire relief by
  `{PARAMS['xh254_socket_relief_expand_toward_sensor_side_y_mm']} mm` toward
