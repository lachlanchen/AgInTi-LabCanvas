#!/usr/bin/env python3
"""Build a direct-socket 25.4 mm C-mount holder for an AS7343 spectral module."""
...
STEM = "as7343_cmount_spectral_module_holder_direct_socket_led_clearance_25p4"
...
def board_reference_geometry() -> dict[str, object]:
    ...
    pin_count = int(PARAMS["pin_header_hole_count"])
    pin_pitch = PARAMS["pin_header_hole_pitch_z_mm"]
    pin_y = bounds["y_min"] + PARAMS["pin_header_hole_y_offset_inside_board_from_negative_y_edge_mm"]
    first_pin_z = -((pin_count - 1) * pin_pitch) / 2.0
    pin_holes = [
        {
            "name": f"pcb_pin_header_{index + 1}",
            "y": round(pin_y, 4),
            "z": round(first_pin_z + index * pin_pitch, 4),
            "cut_diameter_mm": PARAMS["pin_header_hole_diameter_mm"],
        }
    ...
    led_clearance = {
        ...
        "y_min": round(PARAMS["as7343_led_clearance_y_start_mm"], 4),
        "y_max": round(bounds["y_max"] + PARAMS["as7343_led_clearance_to_sensor_side_edge_clearance_mm"], 4),
        "z_min": round(-led_clearance_width_z / 2.0, 4),
        "z_max": round(led_clearance_width_z / 2.0, 4),
        "width_z_mm": PARAMS["as7343_led_clearance_width_z_mm"],
    ...
    led_body_y_max = bounds["y_max"]
    led_body = {
        "name": "as7343_board_led_reference_body",
        "y_min": round(led_body_y_max - PARAMS["as7343_led_body_length_y_mm"], 4),
        "y_max": round(led_body_y_max, 4),
        "z_min": round(-PARAMS["as7343_led_body_width_z_mm"] / 2.0, 4),
        "z_max": round(PARAMS["as7343_led_body_width_z_mm"] / 2.0, 4),
        "body_width_parallel_to_short_edge_z_mm": PARAMS["as7343_led_body_width_z_mm"],
        "body_length_parallel_to_long_edge_y_mm": PARAMS["as7343_led_body_length_y_mm"],
...
def led_clearance_cutter() -> cq.Workplane:
    ref = board_reference_geometry()
    clearance = ref["as7343_led_clearance_relative_to_sensor_mm"]  # type: ignore[index]
    center_y = (clearance["y_min"] + clearance["y_max"]) / 2.0
    center_z = (clearance["z_min"] + clearance["z_max"]) / 2.0
    return x_box(
        (
            sensor_plate_x0() + PARAMS["sensor_plate_thickness_mm"] / 2.0,
            center_y,
            center_z,
        ),
        (
            PARAMS["sensor_plate_thickness_mm"] + 1.2,
            clearance["y_max"] - clearance["y_min"],
            clearance["z_max"] - clearance["z_min"],
        ),
    )


def led_clearance_proxy() -> cq.Workplane:
    ref = board_reference_geometry()
    clearance = ref["as7343_led_clearance_relative_to_sensor_mm"]  # type: ignore[index]
    center_y = (clearance["y_min"] + clearance["y_max"]) / 2.0
    center_z = (clearance["z_min"] + clearance["z_max"]) / 2.0
    return x_box(
        (
            total_length() + PARAMS["board_thickness_mm"] + 0.45,
            center_y,
            center_z,
        ),
        (
...
def as7343_led_proxy() -> cq.Workplane:
    ref = board_reference_geometry()
    led = ref["as7343_led_body_relative_to_sensor_mm"]  # type: ignore[index]
    center_y = (led["y_min"] + led["y_max"]) / 2.0
    center_z = (led["z_min"] + led["z_max"]) / 2.0
    return x_box(
        (
            total_length() + PARAMS["board_thickness_mm"] + 0.45,
            center_y,
...
def build_sensor_plate_body() -> cq.Workplane:
    ...
    holder = holder.cut(led_clearance_cutter()).cut(board_pocket_cutter()).cut(pin_header_hole_clearance_cutter())
    ref = board_reference_geometry()
...
def build_board_proxy() -> cq.Workplane:
    ...
    return board.cut(board_pin_header_hole_cutter()).clean()
...
def build_assembly() -> cq.Assembly:
    ...
    assembly.add(as7343_led_proxy(), name="as7343_board_led_reference_body_near_sensor_edge", color=cq.Color(0.12, 0.90, 0.46, 0.75))
    assembly.add(led_clearance_proxy(), name="as7343_led_clearance_channel_proxy_4mm_wide", color=cq.Color(0.0, 0.75, 0.95, 0.30))
...
def write_alignment_svg(path: Path) -> None:
    ...
    for hole in pin_holes:
        lines.append(circle(hole["y"], hole["z"], hole["cut_diameter_mm"], "#fff5f5", "#c53030", "pin" if hole["name"].endswith("_1") else ""))
    for hole in ref["optional_clamp_holes_relative_to_sensor_mm"]:  # type: ignore[index]
...
def write_readme(path: Path, outputs: dict[str, str]) -> None:
    ...
    pin_rows = "\n".join(
        ...
        for hole in ref["pin_header_holes_relative_to_sensor_mm"]  # type: ignore[index]
    )
    clamp_rows = "\n".join(
        f"| {hole['name']} | `{hole['y']}` | `{hole['z']}` | `{hole['cut_diameter_mm']}` |"
        for hole in ref["optional_clamp_holes_relative_to_sensor_mm"]  # type: ignore[index]
    ...
    path.write_text(
        f"""# AS7343 C-Mount Spectral Module Holder LED Clearance 25.4
...
- Clear the AS7343 board LED with a `{PARAMS['as7343_led_clearance_width_z_mm']} mm`
...
```json
{json.dumps(led_clearance, indent=2)}
```

AS7343 LED reference body:

```json
{json.dumps(led_body, indent=2)}
```
