...
def setup_scene() -> None:
    ...
    mats = {
        ...
        "led": material("green AS7343 board LED reference", (0.12, 0.90, 0.46, 0.78), 0.35, 0.0),
        "led_clearance": material("cyan 4mm LED clearance channel", (0.0, 0.75, 0.95, 0.34), 0.48, 0.0),
    }

    import_stl(CMOUNT_SOCKET_STL, "independent standard 25.4 threaded C-mount socket", mats["cmount"])
    ...
    led_clearance = ref["as7343_led_clearance_relative_to_sensor_mm"]
    led_clearance_y = (led_clearance["y_min"] + led_clearance["y_max"]) / 2.0
    led_clearance_z = (led_clearance["z_min"] + led_clearance["z_max"]) / 2.0
    add_box(
        "4mm LED clearance channel connected to optical bore",
        (
            0.85,
            led_clearance["y_max"] - led_clearance["y_min"],
            led_clearance["z_max"] - led_clearance["z_min"],
        ),
        (total_len + params["board_thickness_mm"] + 0.42, led_clearance_y, led_clearance_z),
        mats["led_clearance"],
    )
    led_body = ref["as7343_led_body_relative_to_sensor_mm"]
    led_y = (led_body["y_min"] + led_body["y_max"]) / 2.0
    led_z = (led_body["z_min"] + led_body["z_max"]) / 2.0
    add_box(
        "AS7343 board LED reference body near sensor-side edge",
        (
            0.95,
            led_body["y_max"] - led_body["y_min"],
            led_body["z_max"] - led_body["z_min"],
        ),
        (total_len + params["board_thickness_mm"] + 0.9, led_y, led_z),
        mats["led"],
    )
...
def main() -> None:
    ...
    total_len = params["female_socket_length_mm"] + params["sensor_plate_thickness_mm"]
    render(RENDER_PATH, (72, -84, 52), (total_len / 2.0, 0, 0), 62, ortho=True, ortho_scale=68)
    render(REAR_RENDER_PATH, (58, 0, 0), (total_len, 0, 0), 70, ortho=True, ortho_scale=58)
