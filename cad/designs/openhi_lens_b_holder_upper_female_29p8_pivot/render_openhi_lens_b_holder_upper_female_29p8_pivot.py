#!/usr/bin/env python3
"""Render the checked OpenHI Lens B 29.8 mm female-receiver variant."""
...
def add_optical_axis(center: Vector, cutaway: bool) -> None:
    ...
    bpy.ops.mesh.primitive_cylinder_add(
        ...
        radius=0.34,
        depth=91.0,
        location=(254.633 - center.x, 210.0 - center.y, 622.5 - center.z),
    )
...
def add_receiver_envelope(manifest: dict, center: Vector) -> None:
    ...
    z0, z1 = receiver["thread_z_range_mm"]
    groove = manifest["thread"]["target_female_groove_mm"]
    bpy.ops.mesh.primitive_cylinder_add(
        vertices=128,
        radius=groove / 2.0,
        depth=z1 - z0,
        location=(254.633 - center.x, 210.0 - center.y, (z0 + z1) / 2.0 - center.z),
    )
...
def load_holder(manifest: dict, *, cutaway: bool):
    ...
    holder.name = "OpenHI Lens B holder, 29.8 mm female pivot"
