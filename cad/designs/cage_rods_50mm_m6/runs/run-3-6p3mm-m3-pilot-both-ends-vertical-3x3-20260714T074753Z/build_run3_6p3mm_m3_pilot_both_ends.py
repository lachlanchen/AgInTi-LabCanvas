#!/usr/bin/env python3
"""Build run 3: 6.30 x 50 mm rods with M3 pilot holes at both ends."""
...
def render_with_blender() -> None:
    ...
    subprocess.run([blender, "--background", "--python", str(RUN_DIR / "render_run3_6p3mm_m3_pilot_both_ends.py")], check=True)
...
def write_readme(path: Path, outputs: dict[str, str], validation: dict[str, object]) -> None:
    ...
    path.write_text(
        f"""# Run 3: 6.30 mm Rods With M3 Pilot Holes
