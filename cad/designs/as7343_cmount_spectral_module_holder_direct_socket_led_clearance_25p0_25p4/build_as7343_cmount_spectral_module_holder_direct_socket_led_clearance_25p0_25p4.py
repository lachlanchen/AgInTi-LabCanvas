#!/usr/bin/env python3
"""Build a direct-socket nominal 25.4 mm C-mount holder for an AS7343 module."""
...
STEM = "as7343_cmount_spectral_module_holder_direct_socket_led_clearance_25p0_25p4"
...
def write_readme(path: Path, outputs: dict[str, str]) -> None:
    ...
    path.write_text(
        f"""# AS7343 C-Mount Spectral Module Holder LED Clearance 25.0/25.4
...
- Use a nominal C-mount receiver size:
  ...
  thread-cutter crest, `0.8 mm` pitch, `{PARAMS['thread_tooth_height_mm']} mm`
  radial tooth height.
...
thread cutter reaches `25.4 mm` nominally. A small `{THREAD_OVERLAP} mm` cutter
