#!/usr/bin/env python3
"""Render the TSL25911 C-mount intensity sensor holder."""
...
def setup_scene() -> None:
    ...
    mats = {
        ...
        "board": material("transparent blue TSL25911 board proxy", (0.02, 0.24, 0.82, 0.68), 0.45),
        "axis": material("warm optical axis", (1.0, 0.74, 0.08, 0.8), 0.24),
        "screw": material("silver M2 screw alignment pins", (0.82, 0.82, 0.78, 0.9), 0.3, 0.4),
        "sensor": material("gold TSL25911 sensing window", (0.72, 0.50, 0.12, 1.0), 0.35, 0.1),
        "connector": material("cream PH2 connector clearance proxy", (0.93, 0.88, 0.72, 0.74), 0.5),
    }

    import_stl(HOLDER_STL, "printed holder with OpenHI female C-mount receiver", mats["holder"])
    import_stl(BOARD_STL, "Waveshare TSL25911 board proxy, window centered", mats["board"])

    total_len = params["female_socket_length_mm"] + params["tube_length_mm"] + params["sensor_plate_thickness_mm"]
    add_x_cylinder("optical axis through C-mount and TSL25911 window", 0.45, total_len + 8.0, (total_len / 2.0 - 1.5, 0, 0), mats["axis"], vertices=48)

    for hole in ref["mounting_holes_relative_to_window_mm"]:
    ...
    add_box("TSL25911 package proxy at optical axis", (1.0, 3.0, 3.6), (total_len + params["board_thickness_mm"] + 0.45, 0, 0), mats["sensor"])
    add_x_cylinder("TSL25911 active window marker", params["tsl25911_window_diameter_mm"] / 2.0, 1.4, (total_len + params["board_thickness_mm"] + 0.1, 0, 0), mats["axis"], vertices=48)
    bounds = ref["board_bounds_relative_to_window_mm"]
    add_box(
        "PH2.0 5-pin connector clearance proxy",
        (6.5, 5.0, 17.0),
        (total_len + params["board_thickness_mm"] + 3.1, bounds["y_max"] + 2.5, 0),
        mats["connector"],
    )
...
def main() -> None:
    setup_scene()
    render(RENDER_PATH, (88, -96, 58), (19, 0, 0), 62, ortho=True, ortho_scale=76)
    render(REAR_RENDER_PATH, (78, 0, 0), (37, 0, 0), 70, ortho=True, ortho_scale=58)
