...
RUN_NAME = "run-2-15mm-table-wings-6p4mm-holes-50mm-pitch-print-ready-20260718T084900Z"
...
PARAMS = {
    ...
    "anti_warp_ear_reason": (
        "Omitted because the 70 x 50 mm footprint is compact and the wings are 5 mm thick."
    ),
}
...
def grid_positions() -> list[tuple[float, float]]:
    ...
    pitch_y = p["overall_width_with_wings_mm"] + p["grid_gap_y_mm"]
    return [
...
def geometry_checks() -> dict[str, object]:
    ...
    return {
        ...
        "grid_pitch_y_mm": p["overall_width_with_wings_mm"] + p["grid_gap_y_mm"],
        "expected_single_bbox_mm": [50.0, 70.0, 15.0],
        "expected_grid_bbox_mm": [105.0, 145.0, 15.0],
    }


def feature_checks(holder: cq.Workplane) -> dict[str, object]:
    p = PARAMS
    shape = holder.val()
...
def main() -> None:
    ...
    validation = {
        ...
        "features": feature_checks(holder),
    ...
    if validation["single_step"]["bbox_mm"] != validation["geometry"]["expected_single_bbox_mm"]:
        raise RuntimeError("Single-holder STEP bounds do not match the design envelope")
    if validation["grid_step"]["solid_count"] != 4:
        raise RuntimeError("2x2 STEP must contain exactly four solids")
    if validation["grid_step"]["bbox_mm"] != validation["geometry"]["expected_grid_bbox_mm"]:
        raise RuntimeError("2x2 STEP bounds do not match the print-grid envelope")
    ...
    for stale_backup in RUN_ARTIFACT_DIR.glob("*.blend1"):
        stale_backup.unlink()
