#!/usr/bin/env python3
"""Render the single cradle, tube fit, cross section, and 2x2 print layout."""
...
def render_grid() -> None:
    clear_scene()
    grid = import_stl(
        ARTIFACT_DIR / f"{STEM}_2x2_print_grid.stl",
        "2x2 direct-print cradle layout",
        (0.08, 0.50, 0.76, 1.0),
    )
    objects = [grid]
    add_floor(objects)
    ...
    _, _, center = bounds(objects)
    render(
        ARTIFACT_DIR / f"{STEM}_2x2_print_grid_render.png",
        objects,
        center + Vector((135.0, -145.0, 125.0)),
        142.0,
    )
