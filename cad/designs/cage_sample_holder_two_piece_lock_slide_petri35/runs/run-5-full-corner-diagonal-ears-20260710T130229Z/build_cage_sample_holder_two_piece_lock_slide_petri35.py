...
PARAMS = {
    ...
    "anti_warp_ear_style": "four removable 0.5 mm thick Y corner ears per printed part; side pulls plus direct diagonal full-corner pull",
    "anti_warp_ear_thickness_mm": 0.5,
    "anti_warp_ear_corner_contact_width_mm": 3.2,
    "anti_warp_ear_diagonal_corner_contact_width_mm": 5.2,
    "anti_warp_ear_breakaway_overlap_mm": 0.35,
    "anti_warp_ear_arm_offset_mm": 3.2,
    "anti_warp_ear_arm_width_mm": 3.2,
    "anti_warp_ear_junction_offset_mm": 7.5,
    "anti_warp_ear_tail_reach_mm": 19.0,
    "anti_warp_ear_tail_width_mm": 13.0,
    "anti_warp_ear_note": "Run-5 keeps the 0.5 mm Z-thick sacrificial layer and adds a direct diagonal bridge from each true corner to the outer tail pad. This pulls the corner down along the diagonal direction, not only along the two adjacent edges.",
...
def anti_warp_corner_ear(sx: int, sy: int, z_min: float) -> cq.Workplane:
    """Build one thin detachable Y tab at a holder corner.

    Run-4 uses the 0.5 mm value as Z thickness, not as skinny neck width. The
    two side contacts are wider and sit directly against the corner, while the
    thin sacrificial layer remains easy to peel or cut from the printed part.
    """
    p = PARAMS
    half_w = p["outer_width_mm"] / 2.0
    half_h = p["outer_height_mm"] / 2.0
    contact = p["anti_warp_ear_corner_contact_width_mm"]
    overlap = p["anti_warp_ear_breakaway_overlap_mm"]
    arm_offset = p["anti_warp_ear_arm_offset_mm"]
    ...
    diagonal_contact = p["anti_warp_ear_diagonal_corner_contact_width_mm"]

    def local_box(u_center: float, v_center: float, u_size: float, v_size: float) -> cq.Workplane:
        return z_box(
            (u_size, v_size, thickness),
            (sx * (half_w + u_center), sy * (half_h + v_center), z_min + thickness / 2.0),
        )

    def local_poly(local_points: list[tuple[float, float]]) -> cq.Workplane:
        points = [(sx * (half_w + u), sy * (half_h + v)) for u, v in local_points]
        return z_poly(points, thickness, z_min)

    # Direct, wider contact on both adjacent sides of the corner. The small
    # overlap fuses into the part edge despite slicer tolerances; removability
    # comes from the 0.5 mm layer thickness.
    ear = local_box((junction - overlap) / 2.0, -arm_offset, junction + overlap, contact)
    ear = ear.union(local_box(-arm_offset, (junction - overlap) / 2.0, contact, junction + overlap))

    # Direct diagonal bridge from the true corner. This is the extra run-5
    # change: it anchors the actual corner outward along the diagonal, rather
    # than relying only on the two side contacts.
    ear = ear.union(
        local_poly(
            [
                (-overlap, diagonal_contact / 2.0),
                (diagonal_contact / 2.0, -overlap),
                (junction + diagonal_contact / 2.0, junction - diagonal_contact / 2.0),
                (junction - diagonal_contact / 2.0, junction + diagonal_contact / 2.0),
            ]
        )
    )

    ear = ear.union(local_box(junction, (junction - arm_offset) / 2.0, arm, junction + arm_offset + arm))
    ear = ear.union(local_box((junction - arm_offset) / 2.0, junction, junction + arm_offset + arm, arm))
    ...
    ear = ear.union(
        local_poly(
            [
                (junction - stem / 2.0, junction + stem / 2.0),
                (junction + stem / 2.0, junction - stem / 2.0),
                (reach + stem / 2.0, reach - stem / 2.0),
                (reach - stem / 2.0, reach + stem / 2.0),
            ]
        )
    )
...
def add_anti_warp_ears(part: cq.Workplane, *, bed_face: str) -> cq.Workplane:
    p = PARAMS
    if not p["anti_warp_ears_enabled"]:
        return part
    if bed_face == "bottom":
        z_min = 0.0
    elif bed_face == "top":
        z_min = p["plate_thickness_mm"] - p["anti_warp_ear_thickness_mm"]
    else:
        raise ValueError(f"unknown anti-warp ear bed face: {bed_face}")

    for sx in (-1, 1):
        for sy in (-1, 1):
            part = part.union(anti_warp_corner_ear(sx, sy, z_min))
    return part
...
def build_bottom_part() -> cq.Workplane:
    ...
    part = add_anti_warp_ears(part, bed_face="bottom")
    return part
...
def build_top_part() -> cq.Workplane:
    ...
    part = add_anti_warp_ears(part, bed_face="top")
    return part


def build_top_part_180deg_print() -> cq.Workplane:
    """Return the top frame already flipped for printing.

    The normal top part has its removable ears on the top face because the user
    prints this part after a 180 degree flip. This export applies that flip in
    CAD and moves the part back to z=0, so the ears contact the build plate.
    """
    return build_top_part().rotate((0, 0, 0), (1, 0, 0), 180).translate((0, 0, PARAMS["plate_thickness_mm"]))
...
def build_print_layout() -> cq.Assembly:
    ...
    assembly.add(build_top_part_180deg_print().translate((0, 92, 0)), name="top_frame_180deg_print_orientation", color=cq.Color(0.18, 0.18, 0.18, 1.0))
    return assembly
...
four outer corners. In the latest run, the weak dimension is the `{p['anti_warp_ear_thickness_mm']} mm`
Z thickness of the sacrificial layer. The ears keep the two side pulls, and
also add a direct diagonal bridge from the true corner to the outer tail pad.
This pulls the actual corner down in the diagonal direction during printing,
not only along the two adjacent edges.
...
def main() -> None:
    ...
    paths = {
        ...
        "top_part_180deg_print_step": ARTIFACT_DIR / f"{STEM}_top_part_180deg_print.step",
        "top_part_180deg_print_stl": ARTIFACT_DIR / f"{STEM}_top_part_180deg_print.stl",
    ...
    export_part(build_top_part_180deg_print(), paths["top_part_180deg_print_step"], paths["top_part_180deg_print_stl"])
