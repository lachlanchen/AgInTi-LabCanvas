...
def anti_warp_corner_ear(sx: int, sy: int, z_min: float) -> cq.Workplane:
    ...
    The local polygon has two narrow necks touching the adjacent sides of the
    holder and a wider diagonal tail outside the corner. The necks are only
    0.5 mm wide so the tab can be snapped or cut away after printing.
    """
    p = PARAMS
    half_w = p["outer_width_mm"] / 2.0
    half_h = p["outer_height_mm"] / 2.0
    neck = p["anti_warp_ear_breakaway_neck_width_mm"]
    ...
    ear = local_box((junction - overlap) / 2.0, -arm_offset, junction + overlap, neck)
    ear = ear.union(local_box(-arm_offset, (junction - overlap) / 2.0, neck, junction + overlap))

    # Stronger arms begin after the 0.5 mm breakaway necks, so the snap/cut line
    # remains at the part wall while the outer tab has enough area to hold down.
...
four outer corners. Each ear attaches through two `{p['anti_warp_ear_breakaway_neck_width_mm']} mm`
