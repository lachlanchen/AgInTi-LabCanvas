#!/usr/bin/env python3
"""Render the two-piece locking sample holder."""
...
TOP_PRINT_STL = ARTIFACT_DIR / f"{STEM}_top_part_180deg_print.stl"
...
P = {
    ...
    "top_z": 26.0,
    "rod_diameter": 5.9,
    "top_rods": [(-15.0, -15.0), (15.0, -15.0), (-15.0, 15.0), (15.0, 15.0)],
    "bottom_rods": [(-15.0, -15.0), (15.0, -15.0), (-15.0, 15.0), (15.0, 15.0)],
...
def add_rods(material) -> None:
    for x, y in P["top_rods"]:
        add_cylinder_z("upper 6 mm rod proxy", P["rod_diameter"] / 2.0, 24.0, (x, y, P["top_z"] + P["plate_thickness"] + 11.5), material)
    for x, y in P["bottom_rods"]:
        add_cylinder_z("lower 6 mm rod proxy", P["rod_diameter"] / 2.0, 24.0, (x, y, -11.5), material)


def add_samples(materials) -> None:
    add_box("OpenHI slide strip proxy", P["slide"], (0, 0, 7.3), materials["slide"])
    add_cylinder_z("33 mm petri dish proxy", P["petri_diameter"] / 2.0, 1.2, (0, 0, 7.15), materials["petri"])
...
def render_assembled() -> None:
    ...
    render(ASSEMBLED_RENDER, (130, -150, 105), (0, 0, 16), 64, ortho=True, ortho_scale=132)


def render_exploded() -> None:
    materials = setup_common()
    import_stl(BOTTOM_STL, "bottom tray with lock feet", materials["bottom"])
    import_stl(TOP_STL, "top frame lifted to show lock holes", materials["top"], (0, 0, 50))
    add_rods(materials["rod"])
    add_box("OpenHI slide strip proxy", P["slide"], (0, -52, 11), materials["slide"])
    add_cylinder_z("33 mm petri dish proxy", P["petri_diameter"] / 2.0, 1.2, (0, 52, 10.4), materials["petri"])
    render(EXPLODED_RENDER, (150, -174, 122), (0, 0, 24), 72, ortho=True, ortho_scale=170)


def render_print_layout() -> None:
    materials = setup_common()
    import_stl(BOTTOM_STL, "print layout bottom part", materials["bottom"], (0, -62, 0))
    import_stl(TOP_PRINT_STL, "print layout top part already 180 degree flipped", materials["top"], (0, 62, 0))
