#!/usr/bin/env python3
"""Build a decoupled OpenHI-fit C-mount holder for an AS7343 spectral module."""
...
STEM = "as7343_cmount_spectral_module_holder_decoupled"
...
def cmount_tube_length() -> float:
    return PARAMS["female_socket_length_mm"] + PARAMS["tube_length_mm"]


def sensor_plate_x0() -> float:
    return cmount_tube_length()
...
def build_holder_compound() -> cq.Compound:
    return cq.Compound.makeCompound([build_cmount_tube_body().val(), build_sensor_plate_body().val()])
...
def build_assembly() -> cq.Assembly:
    ...
    assembly.add(build_cmount_tube_body(), name="independent_openhi_24p8_threaded_cmount_tube", color=cq.Color(0.10, 0.10, 0.09, 1.0))
...
def write_readme(path: Path, outputs: dict[str, str]) -> None:
    ...
    path.write_text(
        f"""# AS7343 C-Mount Spectral Module Holder Decoupled
...
- Export the C-mount/tube and sensor plate as separate adjacent solids; they
  touch at x=`{sensor_plate_x0()}` without a bridge cube or boolean union.
...
def main() -> None:
    ...
    cmount_tube = build_cmount_tube_body()
    ...
    cmount_tube_step = ARTIFACT_DIR / f"{STEM}_cmount_tube.step"
    cmount_tube_stl = ARTIFACT_DIR / f"{STEM}_cmount_tube.stl"
    ...
    exporters.export(cmount_tube, str(cmount_tube_step))
    exporters.export(cmount_tube, str(cmount_tube_stl))
    ...
    outputs = {
        ...
        "cmount_tube_step": repo_path(cmount_tube_step),
        "cmount_tube_stl": repo_path(cmount_tube_stl),
