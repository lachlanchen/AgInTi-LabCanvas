#!/usr/bin/env python3
"""Build run 3: exact 10.00 mm to 6.30 mm adapter with an M3 pilot."""
    ...
    for row in range(int(p["print_grid_rows"])):
        for col in range(int(p["print_grid_cols"])):
            ...
            assembly.add(
                ...
                name=f"m10_to_6p3_m3_pilot_adapter_{index:02d}",
...
def render_with_blender() -> None:
    ...
    subprocess.run([blender, "--background", "--python", str(RUN_DIR / "render_run3_10p0_to_6p3_m3_pilot.py")], check=True)
...
def write_readme(path: Path, outputs: dict[str, str], validation: dict[str, object]) -> None:
    ...
    path.write_text(
        f"""# Run 3: Exact 10.00 mm To 6.30 mm Adapter
