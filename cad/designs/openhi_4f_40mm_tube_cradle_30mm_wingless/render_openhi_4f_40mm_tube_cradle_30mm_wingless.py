#!/usr/bin/env python3
"""Render the compact wingless cradle, tube fit, and print layout."""
...
def render_single() -> None:
    ...
    holder = import_stl(
        ...
        "40.2 mm circular-seat wingless cradle",
        (0.08, 0.50, 0.76, 1.0),
    )
    objects = [holder]
    add_floor(objects)
...
def render_clean_top() -> None:
    clear_scene()
    holder = import_stl(
        ARTIFACT_DIR / f"{STEM}_holder.stl",
        "compact wingless cradle",
        (0.08, 0.50, 0.76, 1.0),
    )
    objects = [holder]
    add_floor(objects)
    ...
    _, _, center = bounds(objects)
    render(
        ARTIFACT_DIR / f"{STEM}_clean_top_render.png",
        objects,
