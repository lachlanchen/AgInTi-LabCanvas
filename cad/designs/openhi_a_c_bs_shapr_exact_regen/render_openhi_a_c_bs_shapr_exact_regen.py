#!/usr/bin/env python3
"""Render the exact-regenerated OpenHI A+C+BS holder."""
...
def setup_scene(stl_path: Path, cutaway: bool = False) -> dict:
    ...
    mat_holder = material("warm gray exact OpenHI A+C+BS holder", (0.66, 0.65, 0.60, 1.0), 0.64)
    mat_axis = material("gold optical axes", (1.0, 0.72, 0.08, 0.9), 0.28)
    mat_thread = material("transparent blue measured 30.2 mm receiver zones", (0.1, 0.35, 1.0, 0.25), 0.48)

    bpy.ops.import_mesh.stl(filepath=str(stl_path))
    holder = bpy.context.object
    holder.name = "OpenHI A+C+BS exact B-rep regeneration"
    ...
    if not cutaway:
        add_z_cylinder(
            "bottom measured 30.2 mm receiver/thread envelope",
            15.1,
            8.1,
            (255.0 - center.x, 210.0 - center.y, 543.6 - center.z),
            mat_thread,
        )
        add_x_cylinder(
            "BS/B-side measured 30.2 mm receiver/thread envelope",
            15.1,
            5.2,
            (272.4 - center.x, 210.0 - center.y, 600.0 - center.z),
            mat_thread,
        )
...
def main() -> None:
    ...
    render(THREAD_RENDER_PATH, (68, -34, 68), (4, 0, 0), 58)
