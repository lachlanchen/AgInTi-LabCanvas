...
STEM = "tsl25911_cmount_intensity_sensor_holder_direct_socket_xh254"
...
def board_reference_geometry() -> dict[str, object]:
    ...
    return {
        ...
        "notes": [
            ...
            "Coordinate convention: Y is the 27 mm long direction between the two short edges; Z is the 20 mm short-edge direction. Sensor datum is on the optical axis at Y=0, Z=0. The connector/socket side is positive Y unless a caliper check says otherwise.",
        ],
    }
...
def xh254_socket_relief_cutter() -> cq.Workplane:
    ...
    width_y = socket["y_max"] - socket["y_min"]
    height_z = socket["z_max"] - socket["z_min"]
...
def build_sensor_proxy() -> cq.Workplane:
    ...
    package = x_box(
        (
            total_x + PARAMS["board_thickness_mm"] + PARAMS["tsl25911_package_thickness_x_mm"] / 2.0,
            0.0,
            0.0,
        ),
        (
    ...
    window = x_cylinder(PARAMS["tsl25911_window_diameter_mm"], 1.2, total_x + PARAMS["board_thickness_mm"] + 0.15)
...
def build_assembly() -> cq.Assembly:
    ...
    assembly.add(build_sensor_proxy(), name="tsl25911_window_centered_on_optical_axis", color=cq.Color(0.95, 0.78, 0.20, 1.0))
...
def write_alignment_svg(path: Path) -> None:
    ...
    legend = [
        ...
        "Red dashed slot: XH2.54 5P socket clearance on connector edge",
...
def write_readme(path: Path, outputs: dict[str, str]) -> None:
    ...
    path.write_text(
        f"""# TSL25911 C-Mount Intensity Sensor Holder Direct Socket XH2.54
