#!/usr/bin/env python3
"""Build a 60 mm tall two-piece locking sample holder.

The previous `cage_sample_holder_openhi_slide_petri35` design is intentionally
left untouched. This variant keeps the same sample footprint as the previous
two-piece design, but makes the finished assembly 60 mm tall.
"""
...
def base_plate(height: float) -> cq.Workplane:
    p = PARAMS
    part = z_box(
        (p["outer_width_mm"], p["outer_height_mm"], height),
        (0, 0, height / 2.0),
    )
...
def cut_bottom_rod_sockets(part: cq.Workplane) -> cq.Workplane:
    ...
    return cut_m3_pilot(part, p["bottom_rod_socket_centers_mm"], -0.1, p["bottom_tray_body_height_mm"] + 0.2)


def cut_top_rod_sockets(part: cq.Workplane) -> cq.Workplane:
    p = PARAMS
    z_min = p["top_frame_body_height_mm"] - p["rod_socket_depth_mm"]
    socket_h = p["rod_socket_depth_mm"] + 0.2
    for x, y in p["top_rod_socket_centers_mm"]:
        part = part.cut(z_cylinder(p["rod_socket_diameter_mm"], socket_h, z_min).translate((x, y, 0)))
    return cut_m3_pilot(part, p["top_rod_socket_centers_mm"], -0.1, p["top_frame_body_height_mm"] + 0.2)
...
def add_lock_feet(part: cq.Workplane) -> cq.Workplane:
    p = PARAMS
    z_min = p["bottom_tray_body_height_mm"]
    for x, y in p["lock_points_mm"]:
...
def cut_bottom_sample_seats(part: cq.Workplane) -> cq.Workplane:
    p = PARAMS
    top_z = p["bottom_tray_body_height_mm"]
    ...
    part = part.cut(z_cylinder(p["optical_window_diameter_mm"], p["bottom_tray_body_height_mm"] + 0.6, -0.3))
...
def build_bottom_part() -> cq.Workplane:
    part = base_plate(PARAMS["bottom_tray_body_height_mm"])
...
def build_top_part() -> cq.Workplane:
    p = PARAMS
    part = base_plate(p["top_frame_body_height_mm"])
    win_x, win_y = p["top_inner_window_mm"]
    part = part.cut(z_box((win_x, win_y, p["top_frame_body_height_mm"] + 1.2), (0, 0, p["top_frame_body_height_mm"] / 2.0)))
...
def top_part_z() -> float:
    return PARAMS["top_frame_z_mm"]


def build_slide_proxy() -> cq.Workplane:
    length, width = PARAMS["openhi_strip_nominal_mm"]
    return z_box((length, width, 1.0), (0, 0, PARAMS["bottom_tray_body_height_mm"] - 0.35))


def build_petri_proxy() -> cq.Workplane:
    return z_cylinder(PARAMS["petri_nominal_diameter_mm"], 1.2, PARAMS["bottom_tray_body_height_mm"] - 1.0)
...
def build_rod_proxies() -> cq.Workplane:
    ...
    rods = None
    for x, y in p["top_rod_socket_centers_mm"]:
        rod = z_cylinder(5.9, 23.0, top_part_z() + p["top_frame_body_height_mm"] - 0.8).translate((x, y, 0))
...
def build_exploded() -> cq.Assembly:
    ...
    assembly.add(build_top_part().translate((0, 0, 78.0)), name="top_frame_with_lock_holes", color=cq.Color(0.17, 0.17, 0.16, 1.0))
...
def write_alignment_svg(path: Path) -> None:
    ...
    legend = [
        "60 mm two-piece locking sample holder",
        "Outer: 110 x 70 mm; sample zone: 80 x 40 mm",
        "Assembly height: 60 mm; top body: 30 mm",
        ...
        "22 mm chamber gap leaves finger room for placing/removing samples",
    ]
...
def write_manifest(path: Path, outputs: dict[str, str]) -> None:
    manifest = {
        ...
        "design_intent": "60 mm tall two-piece wide sample holder with lock feet, 6 mm rod sockets, M3 pilot/thread places, overlapping slide and 35 mm petri seats, and finger access.",
        "parameters": PARAMS,
        "outputs": outputs,
    }
...
def write_readme(path: Path, outputs: dict[str, str]) -> None:
    ...
    path.write_text(
        f"""# 60 mm Two-Piece Locking Cage Sample Holder
...
- Finished assembly height: `{p['assembled_height_mm']} mm`.
- Top frame body: `{p['top_frame_body_height_mm']} mm`, placed from z=`{p['top_frame_z_mm']} mm` to z=`{p['assembled_height_mm']} mm`.
- Bottom tray body: `{p['bottom_tray_body_height_mm']} mm`, with tall lock posts for the interface.
- Bottom printable part height including lock posts: `{p['bottom_print_height_mm']} mm`.
- Top printable part height: `{p['top_print_height_mm']} mm`.
