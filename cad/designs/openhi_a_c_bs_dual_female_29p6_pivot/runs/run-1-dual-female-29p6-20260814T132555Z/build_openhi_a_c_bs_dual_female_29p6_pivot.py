#!/usr/bin/env python3
"""Rebuild only the two A+C+BS female receivers at a 29.6 mm pivot.
...
STEM = "openhi_a_c_bs_dual_female_29p6_pivot"
...
FEMALE_PIVOT = 29.6
FEMALE_GROOVE = FEMALE_PIVOT + 2.0 * TOOTH_RADIAL_HEIGHT
...
def build() -> tuple[cq.Workplane, dict[str, cq.Workplane]]:
    ...
    # The side branch has no lens-seat transition in this five-millimetre zone.
    side_fill = x_cylinder(31.4, 269.9, 5.1, *SIDE_CENTER)
    ...
    *,
    ...
    for face in shape.Faces():
        ...
        for vertex in vertices:
            ...
            diameter = 2.0 * radius
            if FEMALE_PIVOT - 0.2 <= diameter <= FEMALE_GROOVE + 0.2:
...
def validate_export(source: cq.Shape, output_path: Path) -> dict[str, Any]:
    ...
    lower_has_45_degree_transitions = sum(
        near(item["semi_angle_deg"], 45.0, 0.01)
        for item in lower["cone_surfaces"]
    ) >= 2
    checks = {
        ...
        "lower_pivot_is_29p6": any(
            near(value, FEMALE_PIVOT)
            for value in lower["cylinder_diameters_mm"]
        ),
        "lower_groove_is_30p4": near(
            lower["bspline_sample_max_diameter_mm"], FEMALE_GROOVE
        ),
        "side_pivot_is_29p6": any(
            near(value, FEMALE_PIVOT)
            for value in side["cylinder_diameters_mm"]
        ),
        "side_groove_is_30p4": near(
            side["bspline_sample_max_diameter_mm"], FEMALE_GROOVE
        ),
        ...
        "lower_chamfers_are_45_degrees": lower_has_45_degree_transitions,
    }
...
def write_readme(manifest: dict[str, Any]) -> None:
    ...
    lines = [
        ...
        f"- Lower receiver measured `29.6/30.4 mm`: `{checks['lower_pivot_is_29p6'] and checks['lower_groove_is_30p4']}`.",
        f"- Side receiver measured `29.6/30.4 mm`: `{checks['side_pivot_is_29p6'] and checks['side_groove_is_30p4']}`.",
        f"- Thread runouts remain bounded: `{checks['lower_thread_clipped_to_receiver'] and checks['side_thread_clipped_to_receiver']}`.",
        f"- Lower 45-degree chamfers preserved: `{checks['lower_chamfers_are_45_degrees']}`.",
        ...
        f"blender --background --python {repo_path(DESIGN_DIR / 'render_openhi_a_c_bs_dual_female_29p6_pivot.py')}",
        "```",
        "",
    ]
...
def main() -> None:
    ...
    manifest: dict[str, Any] = {
        ...
        "scope": {
            "modified": [
                "A+ C + BS lower/A-branch female receiver",
                "A+ C + BS side/C-branch female receiver",
            ],
        ...
        "thread": {
            "female_pivot_mm": FEMALE_PIVOT,
            "female_groove_mm": round(FEMALE_GROOVE, 6),
            "pitch_mm": PITCH,
