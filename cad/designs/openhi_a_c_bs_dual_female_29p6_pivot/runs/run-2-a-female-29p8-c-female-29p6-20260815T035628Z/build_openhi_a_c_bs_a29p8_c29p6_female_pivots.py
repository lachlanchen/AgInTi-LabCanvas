...
def validate_export(source: cq.Shape, output_path: Path) -> dict[str, Any]:
    ...
    checks = {
        ...
        "changes_from_run1_confined_to_lower_A_receiver": (
            accepted_run_delta["lower_A_material_delta_mm3"] < -1e-3
            and abs(accepted_run_delta["side_C_volume_delta_mm3"]) < 1e-6
            and abs(accepted_run_delta["side_C_area_delta_mm2"]) < 1e-6
        ),
...
def write_readme(manifest: dict[str, Any]) -> None:
    ...
    lines = [
        ...
        f"- Relative to accepted run 1, every change is confined to the lower/A receiver: `{checks['changes_from_run1_confined_to_lower_A_receiver']}`.",
