...
| WeCom bridge | Official AI Bot and `wecom-cli` transports plus an allowlisted external-group GUI relay, verified file sends, per-chat sessions, per-member paper/idea archives, immediate `#daily` research, and LaTeX report recovery | [setup and architecture](docs/WECOM_API_BRIDGE.md), [stable GUI interface](agentic_tools/wecom_agent/docs/GUI_RELAY_INTERFACE.md) |
